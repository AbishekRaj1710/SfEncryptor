...
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QGridLayout, 
                             ...
                             QFileDialog, QMessageBox, QScrollArea, QFrame, QGroupBox)
...
class CryptoWorker(QObject):
        ...
        try:
            ...
            for file_path in files_to_process:
                ...
                try:
                    ...
                    if self.params.get('checksum'):
                        checksum = self._calculate_sha256(file_path)
                        with open(f"{output_file}.sha256", "w", encoding="ascii") as checksum_file:
                            checksum_file.write(f"{checksum}  {file_name}\n")

                    if self.params.get('delete_original'):
                        passes = self.params.get('shredding_passes', 0)
                        if passes > 0:
            ...
            for file_path in files_to_process:
                ...
                try:
                    ...
                    if os.path.exists(checksum_file_path):
                        expected_checksum = self._read_checksum(checksum_file_path)
                        actual_checksum = self._calculate_sha256(output_file)
                        if actual_checksum != expected_checksum:
                            return False, f"Integrity check failed for {file_name}"
                except Exception as e:
    ...
    @staticmethod
    def _read_checksum(checksum_file_path):
        with open(checksum_file_path, 'r', encoding='ascii') as checksum_file:
            value = checksum_file.read().strip().split()[0]
        if len(value) != 64 or any(character not in '0123456789abcdefABCDEF' for character in value):
            raise ValueError("Invalid checksum sidecar")
        return value.lower()

    @staticmethod
    def _secure_delete(file_path, passes):
        size = os.path.getsize(file_path)
        with open(file_path, 'r+b') as target:
...
class CryptoTab(QWidget):
    ...
    def create_ui_elements(self):
        ...
        self.show_password_button.setMaximumWidth(80)
    ...
    def layout_ui_elements(self):
        ...
        self.layout.addLayout(password_layout, row, 1, 1, 3)
        row += 1
        ...
        # Encryption-specific options
        if self.is_encrypt_mode:
            options_group = QGroupBox("Post-processing")
            options_layout = QGridLayout(options_group)
            options_layout.setContentsMargins(12, 18, 12, 10)
            options_layout.setHorizontalSpacing(20)
            options_layout.setVerticalSpacing(8)
            options_layout.addWidget(self.checksum_checkbox, 0, 0)
            options_layout.addWidget(self.delete_original_checkbox, 0, 1)
            options_layout.addWidget(self.secure_shredding_passes_label, 1, 0)
            options_layout.addWidget(self.secure_shredding_passes_spinbox, 1, 1)
            options_layout.setColumnStretch(1, 1)
            self.layout.addWidget(options_group, row, 0, 1, 4)
            row += 1
        ...
        action_layout.addWidget(self.action_button, 2)
        action_layout.addWidget(self.cancel_button)
        action_layout.addWidget(self.reset_button)
        self.layout.addLayout(action_layout, row, 0, 1, 4)
        row += 1
        ...
        if self.is_encrypt_mode:
            self.toggle_shredding_options(self.delete_original_checkbox.checkState().value)
    ...
    def toggle_password_visibility(self, visible):
        """Show or hide the password currently entered in the form."""
        self.password_entry.setEchoMode(
            QLineEdit.EchoMode.Normal if visible else QLineEdit.EchoMode.Password
        )
        self.show_password_button.setText("Hide" if visible else "Show")
    ...
    def cancel_operation(self):
        ...
        if self.worker is not None:
            self.worker.cancel()
            self.cancel_button.setEnabled(False)
            self.file_status_label.setText("Cancellation requested...")

    def reset_form(self):
        """Clear selections and return the tab to its initial state."""
        if self.worker is not None:
            return

        self.input_path_entry.clear()
        self.output_path_entry.clear()
        self.key_file_path_entry.clear()
        self.password_entry.clear()
        self.show_password_button.setChecked(False)
        self.progress_bar.setValue(0)
        self.progress_bar.setVisible(False)
        self.file_status_label.setText("Ready to process files")
    ...
    def start_operation(self):
        ...
        if not os.path.exists(output_path):
            try:
                ...
                QMessageBox.warning(self, "Output Error", f"Unable to create output folder: {error}")
                return
        ...
        operation_params = {
            ...
            'checksum': self.checksum_checkbox.isChecked() if self.is_encrypt_mode else False,
            'delete_original': self.delete_original_checkbox.isChecked() if self.is_encrypt_mode else False,
            'shredding_passes': self.secure_shredding_passes_spinbox.value() if self.is_encrypt_mode else 0
        }
